...
int main()
{
    ...
    sort(pp,pp+n);//,[](const pair<int,int>&p1,const pair<int,int>&p2){p1.first<=p2.first;});
   
    vector<ll>v1,v2;
    for(ll i=0;i<n;i++){
    	v1.push_back(pp[i].ff);
    	v2.push_back(pp[i].ss);
    }
